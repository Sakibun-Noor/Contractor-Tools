...
## 3.5 Build log — round-2 feedback, 2026-09-01

Source: `The footer became too long.docx`. Three items, all applied and verified.

#### H-08 · Footer social icons 4-across ✅
**Doc** "Keep the last 4 logos on the right side of the upper 4 logos so the length of the
footer reduces."
**Now** The eight icons from `H-07` were stacked `grid-cols-2` — 4 rows — making the
Follow Us column roughly twice the height of its neighbours and stretching the footer.
**Fix** `grid-cols-2` → `grid-cols-4`. Two rows of four.
**Verified** 8 icons, 2 rows, and all five footer columns now measure the same 189px.

#### SR-10 · Search Results locked to one screen ✅
**Doc** "The search result page is scrollable but I want it to be like rest of the pages
where I do not scroll, only the footer appears on scroll. So fit everything in one screen
except the footer."
**Now** `results.html` was built to flow and scroll — a deliberate choice at the time,
since panel + table don't naturally fit one viewport. The client wants consistency with
the other pages instead.
**Fix** Adopted the same `.page-above-fold` 100dvh shell the other pages use. `main` is a
5-row grid; the table takes the leftover `minmax(0,1fr)` and scrolls inside its own
container. Below 1199px it reverts to natural flow, matching the other pages' behaviour
at their breakpoints. Sticky table header added, since the table now scrolls internally.
**Verified** The fold measures exactly the viewport height; `main` no longer overflows;
the pagination bar is visible; the **only** thing below the fold is the footer.

> **A bug this surfaced.** `.chips-row.is-empty` used `display:none`, which removes the
> element from the grid entirely — so with no active filters every row shifted up a track,
> the table landed in an `auto` row and **collapsed the pagination bar to 0px**. Now
> `visibility:hidden;height:0`, keeping its slot, which is the pattern
...
**Fix** Vendor card logo `28–36px` → `56–76px` (renders 70px). In-table vendor icons
`16/18px` → `22px` across all three results tables.
**Verified** Procore card logo renders 70×70 from a 64px source.

**Partially deliverable — see 4.18.** The mockup shows a full **wordmark** ("PROCORE"
logotype filling the card). What we have are **favicons**: square marks, and roughly half
are low-resolution. Sizing them to full card width would visibly pixelate most vendors, so
the increase was capped where the majority still look clean.
...
**4.18 — Vendor wordmark logos don't exist (⛔ caps V-04)**
`V-04` asks for large vendor logos like the PROCORE wordmark in the mockup. What the site
has is **favicons** — square marks fetched from each vendor's site — not horizontal brand
logotypes. They are different assets, and `logo_url_or_file` reads "Research Needed" for
all 1,463 rows.

Resolution of the 620 cached icons:
...
**Roughly half are under 48px.** At the mockup's size (~280px wide) most would be visibly
pixelated, so `V-04` was capped at 76px where the majority still render cleanly.
*Options:* (a) client supplies real logo files or a logo URL per vendor — the only route to
the mockup; (b) fetch higher-resolution marks where a vendor exposes one
(`apple-touch-icon`, `og:image`) — would help some, not all; (c) accept the capped size.
*Default:* (c) until logo assets exist. **Raise with the client.**
